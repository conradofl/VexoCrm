...
- **FATIA 2 — FEITA (2026-07-17):** montador inline no wizard (`components/geracaoDigital/ProposalWizard.tsx`). Step "Pacotes a Ofertar" ganhou botão "Criar pacote pra esta proposta" → form (nome, período, valor, multiselect de `gdProducts`) → POST `ad_hoc=true` → `onPackageCreated` injeta em `availablePackages` da página + entra em `newPacotesOfertados`/`newPackageId`. Pacote some da biblioteca de Modelos. Props novas no wizard: `clientId`, `getIdToken`, `onPackageCreated`.
  - **Edição de proposta com pacote ad_hoc — CORRIGIDO (2026-07-17).** GET `/api/gd/packages?ids=a,b,c` faz lookup por id sem filtrar ad_hoc; a página, ao carregar propostas (`loadReferencedPackages`), busca os pacotes referenciados (`package_id`/`package_vexo_id`/`pacotes_ofertados`) e mescla em `availablePackages`, então o painel de edição/cálculo reencontra o ad_hoc. Pra não poluir a grade "Pacotes a Ofertar" do wizard de proposta NOVA, ela filtra `!ad_hoc || newPacotesOfertados.includes(id)` (mostra só Modelos + o ad_hoc desta proposta).
  - Opcional "salvar como modelo" (PUT `ad_hoc=false`) ainda não implementado — melhoria futura.
