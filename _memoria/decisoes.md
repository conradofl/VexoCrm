...
- **Redesign de pacotes decidido (a construir): modelo de 2 camadas + snapshot.** Pacote deixa de ser sempre linha na biblioteca global (`gd_packages`) — isso lotava a tela e forçava prefixo de letra por segmento. Novo: (1) Modelos de Pacote reutilizáveis (com coluna `segmento` real + filtro + arquivar); (2) montador inline na proposta que grava snapshot em `gd_proposals.pacotes_ofertados` (JSONB já existe), sem criar linha na biblioteca; botão "salvar como modelo" opcional. Snapshot preserva preço de propostas antigas. PENDENTE de implementação (fila depois do dashboard).
