...
export function registerAllDomainRoutes(app) {
  ...
  app.get("/api/followup-queue", requireFirebaseAuth, async (req, res) => {
    ...
    if (!companyId) return sendError(res, 400, "INVALID_QUERY", "Missing companyId");
    ...
    try {
      ...
      let idx = 2;
      let extra = "";
      if (campaignId) { params.push(campaignId); extra += ` AND fc.id = $${idx++}`; }
      if (dateFrom)   { params.push(dateFrom);   extra += ` AND fs.created_at >= $${idx++}`; }
      if (dateTo)     { params.push(dateTo);      extra += ` AND fs.created_at <= $${idx++}`; }
