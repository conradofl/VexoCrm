-- Força atualização dos prompts padrao/campanha/extrato para outlier e infinie.
-- Usa ON CONFLICT DO UPDATE para sobrescrever linhas vazias ou desatualizadas.
-- Só atualiza se o content atual estiver vazio ou for menor que 100 chars (não sobrescreve customizações reais).

-- ─── OUTLIER — Prompt Padrão ──────────────────────────────────────────────────
...
ON CONFLICT (client_id, type) DO UPDATE
  ...
  WHERE length(chatbot_prompts.content) < 100;

-- ─── OUTLIER — Prompt Campanha ────────────────────────────────────────────────
INSERT INTO chatbot_prompts (client_id, type, content, updated_at, updated_by_email)
VALUES (
  'outlier',
  'campanha',
