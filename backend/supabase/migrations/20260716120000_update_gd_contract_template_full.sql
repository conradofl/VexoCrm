-- Substitui o template padrão minimalista pelo modelo jurídico completo,
-- baseado no contrato real usado pela Geração Digital. Dados da CONTRATADA
-- (Caio/GD) ficam FIXOS no texto; o restante são merge fields {{campo}}
-- preenchidos pelo formulário.
--
-- A plataforma Vexo OS aparece como PRODUTO/licença fornecida pela Contratada
-- (Cláusula Terceira) — nunca como uma segunda empresa/parte. A relação
-- GD <-> Vexo é tratada em contrato separado, invisível ao cliente.
--
-- Idempotente: atualiza os templates que estão com o conteúdo padrão gerado
-- automaticamente (nome antigo ou o nome novo).
