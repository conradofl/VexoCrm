...
export default function GeracaoDigitalPaymentTerms() {
  ...
  return (
    ...
    >
      <GeracaoDigitalTabs />
      <div className="w-full bg-white text-slate-800 rounded-3xl p-6 border border-slate-200 shadow-sm space-y-6">
        <div className="flex justify-between items-center">
          <h3 className="text-xs font-mono font-bold text-purple-600 uppercase tracking-widest">Condições Salvas</h3>
        ...
        {showForm && (
          <Card className="bg-slate-50 border-purple-200">
            <CardHeader className="flex flex-row justify-between items-center space-y-0">
              ...
              <Button variant="ghost" size="icon" onClick={() => setShowForm(false)} className="h-7 w-7 text-slate-500">
                <X className="h-4 w-4" />
              </Button>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-1">
                  <Label className="text-[10px] text-slate-500 font-mono">Nome (texto livre)</Label>
                  <Input
                    value={form.nome}
              ...
              <div className="grid gap-4 md:grid-cols-3">
                ...
                {form.tipo === "entrada_parcelas" && (
                  <>
                    <div className="space-y-1">
                      <Label className="text-[10px] text-slate-500 font-mono">Valor da entrada (R$)</Label>
                      <Input
                        type="number"
                    ...
                    <div className="space-y-1">
                      <Label className="text-[10px] text-slate-500 font-mono">Nº de parcelas</Label>
                      <Input
                        type="number"
                ...
                {(form.tipo === "avista_desconto" || form.tipo === "entrada_parcelas") && (
                  <div className="space-y-1">
                    <Label className="text-[10px] text-slate-500 font-mono">Meio de pagamento</Label>
                    <select
                ...
                {form.tipo === "custom" && (
                  <div className="space-y-1 md:col-span-3">
                    <Label className="text-[10px] text-slate-500 font-mono">Descrição da condição</Label>
                    <textarea
                      value={form.config.descricao ?? ""}
                      onChange={(e) => updateConfig("descricao", e.target.value)}
                      className="w-full min-h-[70px] bg-white border border-slate-200 rounded-lg p-3 text-xs text-slate-800"
              ...
              <div className="p-3 rounded-lg bg-purple-50 border border-purple-100">
                <span className="text-[9px] text-purple-600 font-mono font-bold uppercase tracking-wider block mb-1">
                  Prévia (sobre um total de exemplo de R$ {EXEMPLO_TOTAL.toLocaleString("pt-BR")})
                </span>
                {previewBreakdown.linhas.map((linha, idx) => (
                  <p key={idx} className="text-xs text-slate-700 font-medium">{linha}</p>
                ))}
              </div>
        ...
        {isLoading ? (
          ...
          <Card className="bg-white border-slate-200 text-center py-10">
            <CardContent className="space-y-2">
              <CreditCard className="h-10 w-10 text-slate-400 mx-auto" />
              <p className="text-xs text-slate-500">Nenhuma condição cadastrada. Crie a primeira para ofertar nas propostas.</p>
            </CardContent>
          </Card>
        ) : (
              ...
              return (
                <Card
                  ...
                  className={cn(
                    ...
                    term.ativo ? "bg-white border-slate-200" : "bg-slate-50 border-slate-200 opacity-60"
                  )}
                >
                  <CardHeader className="pb-2 space-y-1">
                    <div className="flex justify-between items-start gap-2">
                      <CardTitle className="text-sm font-black text-slate-800 leading-tight">{term.nome}</CardTitle>
                  ...
                  <CardContent className="space-y-3">
                    ...
                    <div className="flex items-center justify-between gap-1">
                      <div className="flex items-center gap-1.5">
                        ...
                        <span className="text-[9px] text-slate-500 font-mono">{term.ativo ? "ativa" : "inativa"}</span>
                      </div>
