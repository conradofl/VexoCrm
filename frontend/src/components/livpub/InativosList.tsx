...
import { Search, Users, UserMinus, Check, X, Loader2, User, Phone, MessageSquare, Zap, Pencil } from "lucide-react";
...
export function InativosList({ companyId }: InativosListProps) {
  ...
  const handleRunEngine = async () => {
    try {
      ...
      toast.success("Esteira 4 acionada! O motor está varrendo a base — as sugestões aparecem aqui em instantes.");
    } catch (err) {
      ...
      <Card className="border-border bg-card">
        ...
        <CardContent className="text-xs text-muted-foreground space-y-3 leading-relaxed">
          ...
          >
            ...
            {runEngineMutation.isPending ? "Acionando..." : "Ativar Esteira 4"}
          </Button>
        </CardContent>
      </Card>
    </div>
