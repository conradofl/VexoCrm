-- Align public.leads_outlier with public.leads column set (excluding tipo_cliente, faixa_consumo,
-- cidade, estado — those live under chatbot JSON `dados` when needed).
-- Migrates legacy temperature stored in column `status` + constraint leads_outlier_status_check
-- into lead_temperature; recreates `status` as nullable generic TEXT (same role as leads.status).
